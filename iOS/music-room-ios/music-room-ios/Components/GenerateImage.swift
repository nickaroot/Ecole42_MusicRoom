...
public let deviceColorSpace: CGColorSpace = {
    guard #available(iOSApplicationExtension 9.3, iOS 9.3, *) else {
...
public func generateImagePixel(
    _ size: CGSize,
    scale: CGFloat,
    pixelGenerator: (CGSize, UnsafeMutablePointer<UInt8>, Int) -> Void
) -> UIImage? {
    let context = DrawingContext(size: size, scale: scale, opaque: false, clear: false)
    pixelGenerator(
        CGSize(width: size.width * scale, height: size.height * scale),
        context.bytes.assumingMemoryBound(to: UInt8.self),
        context.bytesPerRow
    )
...
private func withImageBytes(image: UIImage, _ f: (UnsafePointer<UInt8>, Int, Int, Int) -> Void) {
    ...
    let scaledSize = CGSize(
        ...
        height: image.size.height * selectedScale
    )
    let bytesPerRow = DeviceGraphicsContextSettings.shared.bytesPerRow(
        forWidth: Int(scaledSize.width)
    )
...
public func generateGrayscaleAlphaMaskImage(image: UIImage) -> UIImage? {
    ...
    withImageBytes(
        image: image,
        { pixels, width, height, imageBytesPerRow in
...
public func generateImage(
    _ size: CGSize,
    contextGenerator: (CGSize, CGContext) -> Void,
    opaque: Bool = false,
    scale: CGFloat? = nil
) -> UIImage? {
...
public func generateImage(
    ...
    opaque: Bool = false,
    scale: CGFloat? = nil,
    rotatedContext: (CGSize, CGContext) -> Void
) -> UIImage? {
    if size.width.isZero || size.height.isZero {
        return nil
    }
...
public func generateFilledCircleImage(
    diameter: CGFloat,
    color: UIColor?,
    ...
    return generateImage(
        ...
        contextGenerator: { size, context in
            ...
            if let strokeColor = strokeColor, let strokeWidth = strokeWidth {
                ...
                context.fillEllipse(
                    in: CGRect(
                        origin: CGPoint(x: strokeWidth, y: strokeWidth),
                        size: CGSize(
                            width: size.width - strokeWidth * 2.0,
                            height: size.height - strokeWidth * 2.0
                        )
                    )
                )
            } else {
...
public func generateAdjustedStretchableFilledCircleImage(
    diameter: CGFloat,
    color: UIColor
) -> UIImage? {
    let corner: CGFloat = diameter / 2.0
    return generateImage(
        CGSize(width: diameter + 2.0, height: diameter + 2.0),
        contextGenerator: { size, context in
            context.clear(CGRect(origin: CGPoint(), size: size))
            context.setFillColor(color.cgColor)
            ...
            context.addArc(
                ...
                tangent2End: CGPoint(x: corner, y: 0.0),
                radius: corner
            )
            context.addLine(to: CGPoint(x: size.width - corner, y: 0.0))
            context.addArc(
                tangent1End: CGPoint(x: size.width, y: 0.0),
                tangent2End: CGPoint(x: size.width, y: corner),
                radius: corner
            )
            ...
            context.addArc(
                ...
                tangent2End: CGPoint(x: 0.0, y: size.height - corner),
                radius: corner
            )
            context.closePath()
            context.fillPath()
    ...
    return generateImage(
        ...
        contextGenerator: { size, context in
            ...
            context.drawLinearGradient(
                gradient,
                start: CGPoint(),
                end: CGPoint(x: 0.0, y: bounds.size.height),
...
public func generateCircleImage(
    diameter: CGFloat,
    lineWidth: CGFloat,
    color: UIColor?,
    backgroundColor: UIColor? = nil
    ...
    return generateImage(
        ...
        contextGenerator: { size, context in
            ...
            context.strokeEllipse(
                in: CGRect(
                    origin: CGPoint(x: lineWidth / 2.0, y: lineWidth / 2.0),
...
public func generateStretchableFilledCircleImage(
    radius: CGFloat,
    color: UIColor?,
    backgroundColor: UIColor? = nil
) -> UIImage? {
    ...
    return generateFilledCircleImage(
        diameter: radius * 2.0,
        color: color,
        backgroundColor: backgroundColor
    )?
        .stretchableImage(withLeftCapWidth: cap, topCapHeight: cap)
}
...
public func generateVerticallyStretchableFilledCircleImage(
    radius: CGFloat,
    color: UIColor?,
    backgroundColor: UIColor? = nil
) -> UIImage? {
    return generateImage(
        CGSize(width: radius * 2.0, height: radius * 2.0 + radius),
        contextGenerator: { size, context in
            context.clear(CGRect(origin: CGPoint(), size: size))
            if let backgroundColor = backgroundColor {
                context.setFillColor(backgroundColor.cgColor)
            ...
            context.fillEllipse(
                in: CGRect(
                    origin: CGPoint(),
                    size: CGSize(width: radius + radius, height: radius + radius)
                )
            )
            context.fillEllipse(
                in: CGRect(
...
public func generateSmallHorizontalStretchableFilledCircleImage(
    diameter: CGFloat,
    color: UIColor?,
    backgroundColor: UIColor? = nil
) -> UIImage? {
    return generateImage(
        CGSize(width: diameter + 1.0, height: diameter),
        contextGenerator: { size, context in
            context.clear(CGRect(origin: CGPoint(), size: size))
            
            if let subImage = generateImage(
                CGSize(width: diameter + 1.0, height: diameter),
                contextGenerator: { size, context in
                    context.clear(CGRect(origin: CGPoint(), size: size))
                    ...
                    context.fill(
                        CGRect(
                            origin: CGPoint(x: diameter / 2.0, y: 0.0),
                            size: CGSize(width: 1.0, height: diameter)
                        )
                    )
                    context.fillEllipse(
                        in: CGRect(
                ...
                if let cgImage = subImage.cgImage {
                    context.clip(to: CGRect(origin: CGPoint(), size: size), mask: cgImage)
                    context.fill(CGRect(origin: CGPoint(), size: size))
                }
            }
        }
...
public func generateTintedImage(
    image: UIImage?,
    color: UIColor,
    backgroundColor: UIColor? = nil
) -> UIImage? {
    ...
    if let cgImage = image.cgImage, let context = UIGraphicsGetCurrentContext() {
        if let backgroundColor = backgroundColor {
            context.setFillColor(backgroundColor.cgColor)
...
public func generateGradientImage(
    size: CGSize,
    colors: [UIColor],
    locations: [CGFloat],
    direction: GradientImageDirection = .vertical
) -> UIImage? {
    ...
    if let context = UIGraphicsGetCurrentContext() {
        ...
        if let gradient = CGGradient(
            ...
            context.drawLinearGradient(
                ...
                ? CGPoint(x: size.width, y: 0.0) : CGPoint(x: 0.0, y: size.height),
                options: CGGradientDrawingOptions()
            )
        }
    }
...
public func generateScaledImage(
    image: UIImage?,
    size: CGSize,
    opaque: Bool = true,
    scale: CGFloat? = nil
        ...
        init(
            ...
            if #available(iOS 10.0, *) {
                if UIScreen.main.traitCollection.displayGamut == .P3 {
                    self.colorSpace =
                    CGColorSpace(name: CGColorSpace.displayP3) ?? context.colorSpace
...
//            assert(self.bitsPerComponent == 8)
    ...
    if #available(iOS 10.0, *) {
        ...
        let opaqueRenderer = UIGraphicsImageRenderer(
            bounds: CGRect(origin: CGPoint(), size: CGSize(width: 1.0, height: 1.0)),
            format: opaqueFormat
        )
        ...
        let transparentRenderer = UIGraphicsImageRenderer(
            bounds: CGRect(origin: CGPoint(), size: CGSize(width: 1.0, height: 1.0)),
        ...
        if let refContext = UIGraphicsGetCurrentContext() {
        ...
        if let refCtxTransparent = UIGraphicsGetCurrentContext() {
    ...
    guard let opaqueSettings = opaqueSettings,
          let transparentSettings = transparentSettings
    else {
        return DeviceGraphicsContextSettings.shared
    }
...
public struct DeviceGraphicsContextSettings {
    ...
    getSharedDevideGraphicsContextSettings()
...
public class DrawingContext {
    ...
    public init(
        ...
        self.bytesPerRow = DeviceGraphicsContextSettings.shared.bytesPerRow(
            forWidth: Int(scaledSize.width)
        )
    ...
    public func generateImage() -> UIImage? {
        guard !scaledSize.width.isZero, !scaledSize.height.isZero,
        ...
        guard let dataProvider = self.imageBuffer.createDataProviderAndInvalidate(),
              let image = CGImage(
                width: Int(self.scaledSize.width),
                height: Int(self.scaledSize.height),
    ...
    public func generatePixelBuffer() -> CVPixelBuffer? {
        ...
        CVPixelBufferCreateWithBytes(
            nil,
            Int(self.scaledSize.width),
            Int(self.scaledSize.height),
    ...
    public func colorAt(_ point: CGPoint) -> UIColor {
        ...
        guard x >= 0 && x < Int(self.scaledSize.width) && y >= 0 && y < Int(self.scaledSize.height)
        else {
            return UIColor.clear
        }
        let srcLine = self.bytes.advanced(by: y * self.bytesPerRow)
    ...
    public func blt(_ other: DrawingContext, at: CGPoint, mode: DrawingContextBltMode = .alpha) {
        if abs(other.scale - self.scale) < CGFloat.ulpOfOne {
            ...
            let width = min(
                Int(self.size.width * self.scale) - dstX,
                Int(other.size.width * self.scale)
            )
            let height = min(
                Int(self.size.height * self.scale) - dstY,
                Int(other.size.height * self.scale)
            )
            ...
            case .alpha:
                while dstY < maxDstY {
                    let srcLine = other.bytes.advanced(by: max(0, srcY) * other.bytesPerRow)
                        .assumingMemoryBound(to: UInt32.self)
                    let dstLine = self.bytes.advanced(by: max(0, dstY) * self.bytesPerRow)
                        .assumingMemoryBound(to: UInt32.self)
...
extension UIImage {
    ...
    public var cmSampleBuffer: CMSampleBuffer? {
        ...
        if let newSampleBuffer = newSampleBuffer,
           ...
           ), let dict = (attachments as NSArray)[0] as? NSMutableDictionary
        {
            
            dict.setValue(
                kCFBooleanTrue as AnyObject,
                forKey: kCMSampleAttachmentKey_DisplayImmediately as NSString as String
            )
        }
...
public func readCGFloat(
    _ index: inout UnsafePointer<UInt8>,
...
public func floorToScreenPixels(_ value: CGFloat) -> CGFloat {
    floor(((value + CGFloat.ulpOfOne) * UIScreen.main.scale) / UIScreen.main.scale)
}

public func ceilToScreenPixels(_ value: CGFloat) -> CGFloat {
    round((value * UIScreen.main.scale) / UIScreen.main.scale)
}
...
public func drawIcon(
    ...
    if let cgImage = icon.cgImage {
        ...
        context.clip(to: CGRect(
            origin: CGPoint(
                x: floorToScreenPixels(size.width / 2)
                - floorToScreenPixels(iconSize.width / 2),
                y: floorToScreenPixels(size.height / 2)
                - floorToScreenPixels(iconSize.height / 2)
            ),
            size: iconSize
        ), mask: cgImage)
